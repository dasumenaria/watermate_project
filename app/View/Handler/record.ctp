	...
	<form method="get">
		<div class="col-sm-12">
		 ...
		 <input type="hidden" name="village_id" value="<?php echo $village_id; ?>" />
			<button type="submit" class="btn btn-sm btn-primary pull-right print" name="complete_data" value="Watermate" formaction="complete_excel"  style="margin-right: 5px;"><i class="fa fa-download"></i> Excel</button>
		</div>
	</form>
	<div class="col-sm-12">
...
foreach($records as $data_record)
{
			...
			$find_district=$this->requestAction(array('controller' => 'Handler', 'action' => 'find_district',$find_block[0]['block']['district_id']), array());
				...
				<tr>
					...
					<td><?php  if($status=='O'){ echo $ifr=800/1000; } else { echo '-'; } ?></td>
					<td><?php echo $pfr=number_format($data_record['record']['pfr']/1000 ,2); ?></td>
